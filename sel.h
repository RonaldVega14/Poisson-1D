...
void applyNeumann(mesh &m, Vector &b)
{
    for (int i = 0; i < m.getSize(NEUMANN); i++)
    {
        ...
        //Sumandole a b en la casilla i el valor de la condicion de NEUMANN
...
void applyDirichlet(mesh &m, Matrix &K, Vector &b)
{
    for (int i = 0; i < m.getSize(DIRICHLET); i++)
    {
        ...
        int index = c.getNode1() - 1; //Este index representa en que nodo se va a aplicar la condicion

        K.erase(K.begin() + index); //Eliminar fila de K que correspondan a index.
        b.erase(b.begin() + index); //Eliminar fila de b.

        //Eliminando columna y agregando los valores al vector b
